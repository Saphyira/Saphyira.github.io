//GLOBAL VARIABLES
let player1;
let player2;
let player3;
let player4;

//gameStart response:
let players;
let currentTopcard;
let currentPlayer;
let gameID;

let reloadPlayer1Required = false;
let reloadPlayer2Required = false;
let reloadPlayer3Required = false;
let reloadPlayer4Required = false;

let currentWildCardColor;
let currentCardValue;
let currentCardColor;

let nameArray;

//EVENTHANDLER
//modal anzeigen
$(document).ready(function() {
    $('#modal-names').modal('show');
    $("#modal-new-color").modal('hide');
});
//clickhandler
$("#button-start-game").click(createPlayers);

$("#button-change-color").click(chooseNewColor);

$("#button-draw").click(drawCard);
$("#stack img").click(drawCard);

$("#player-handcards").on("click", "img", function() {

      $("img").removeClass('wrong-card');
      $("ul").removeClass("wrong-player");

    if ("handcards1" == $(this).closest("ul").attr("id") && currentPlayer == players[0].Player) {
        if (!$(this).hasClass("wrong-card")) {
          $(this).addClass("wrong-card");
        }
        console.log("Player 1 clicked on Card.");
        currentCardColor = $(this).data("color");
        currentCardValue = $(this).data("value");
        checkCard($(this).data("color"));
    } else if ("handcards2" == $(this).closest("ul").attr("id") && currentPlayer == players[1].Player) {
        if (!$(this).hasClass("wrong-card")) {
          $(this).addClass("wrong-card");
        }
        console.log("Player 2 clicked on Card.");
        currentCardColor = $(this).data("color");
        currentCardValue = $(this).data("value");
        checkCard($(this).data("color"));
    } else if ("handcards3" == $(this).closest("ul").attr("id") && currentPlayer == players[2].Player) {
        if (!$(this).hasClass("wrong-card")) {
          $(this).addClass("wrong-card");
        }
        console.log("Player 3 clicked on Card.");
        currentCardColor = $(this).data("color");
        currentCardValue = $(this).data("value");
        checkCard($(this).data("color"));

    } else if ("handcards4" == $(this).closest("ul").attr("id") && currentPlayer == players[3].Player) {
        if (!$(this).hasClass("wrong-card")) {
          $(this).addClass("wrong-card");
        }
        console.log("Player 4 clicked on Card.");
        currentCardColor = $(this).data("color");
        currentCardValue = $(this).data("value");
        checkCard($(this).data("color"));

    } else {
        console.log("Player wanted to play another players cards", $(this).closest("ul").attr("id"));
        console.log("Current Player", currentPlayer);
        if (!$(this).closest("ul").hasClass('wrong-player')) {
            $(this).closest("ul").addClass("wrong-player");
        }
    }
});

//FUNCTIONS
function checkCard(cardColor) {
    $("ul").removeClass("wrong-player");
    $("#player-handcards ul").removeClass("wrong-card");

    if ("Black" == cardColor) {
        $("#modal-new-color").modal('show');
    } else {
        playCard(currentCardValue, currentCardColor, "");
    }
}

function chooseNewColor() {
    currentWildCardColor = $("#modal-new-color").find("option:selected").text();
    console.log("New Color = ", currentWildCardColor);

    $("#modal-new-color").modal('hide');
    playCard(currentCardValue, currentCardColor, currentWildCardColor);
}

function createPlayers() {
    //prüfen, ob felder ausgefüllt sind
    if ('' != $("#player1").val() && '' != $("#player2").val() && '' != $("#player3").val() && '' != $("#player4").val()) {
        //liste von allen spielern erstellen

        let inputFields = $("#modal-names input:text");

        // i = index 1, k = index 2
        for (let i = 0; i < 4; i++) {
            //val() --> nur auf $-elemente anwendbar! hier direkt aufs DOM-Element zugreifen besser!!
            console.log(inputFields[i].value);
            let counter = 0;
            for (let k = 0; k < 4; k++) {
                if (inputFields[i].value == inputFields[k].value) {
                    counter++;
                }
                //püft, ob es zwei gleiche werte/namen gibt
                if (2 <= counter) {
                    console.log("Douplicate found, try again.");
                    //wenn es den alert schon gibt - nicht neu erstellen
                    if (!$('#modal-names .modal-body:last').hasClass('alert')) {
                        let appendAlert = $("#modal-names .modal-body:last");
                        appendAlert.append("<div id='errorUserNames' class='text-center alert'> Douplicated names found </div>")
                    }
                    return;
                }
            }
        }
        console.log("No douplicates.");

        player1 = $("#player1").val();
        console.log("Player 1 " + player1 + " joined the Game");
        player2 = $("#player2").val();
        console.log("Player 2 " + player2 + " joined the Game");
        player3 = $("#player3").val();
        console.log("Player 3 " + player3 + " joined the Game");
        player4 = $("#player4").val();
        console.log("Player 4 " + player4 + " joined the Game");

        nameArray = inputFields.map(function() {
            return this.value;
        }).get();

        gameStart(nameArray);

        $('#modal-names').modal('hide');

    } else {
        console.log("Nicht alle Namens-Felder waren ausgefüllt.");

    }
}

function currentPlayerReloadRequired() {
    if (currentPlayer == players[0].Player) {
        reloadPlayer1Required = true;
    } else if (currentPlayer == players[1].Player) {
        reloadPlayer2Required = true;
    } else if (currentPlayer == players[2].Player) {
        reloadPlayer3Required = true;
    } else if (currentPlayer == players[3].Player) {
        reloadPlayer4Required = true;
    }
}

function reloadPlayerNames() {
    console.log("Player 1: ", $("#button-player-1").text());
    let playerName1 = $("#button-player-1").text()
    $("#button-player-1").text(playerName1 + " " + players[0].Player);

    let playerName2 = $("#button-player-2").text()
    $("#button-player-2").text(playerName2 + " " + players[1].Player);

    let playerName3 = $("#button-player-3").text()
    $("#button-player-3").text(playerName3 + " " + players[2].Player);

    let playerName4 = $("#button-player-4").text()
    $("#button-player-4").text(playerName4 + " " + players[3].Player);
}


function reloadPlayerBadge() {

  $("#player-handcards span").remove();
  $(".card").removeClass("current-player");

  if ($("#player-handcards button").hasClass("btn-success")) {
    $("#player-handcards button").removeClass("btn-success");
    $("#player-handcards button").addClass("btn-warning");
  }

  if (currentPlayer == players[0].Player) {
    if ($("#button-player-1").hasClass("btn-warning")) {
      $("#button-player-1").removeClass("btn-warning");
      $("#button-player-1").addClass("btn-success");
      $("#button-player-1").closest(".card").addClass("current-player");
    }
    $("<span class='badge badge-light ml-2'>It's your turn!</span>").appendTo('#button-player-1');
    reloadPlayer1Required = true;
    reloadHandCards();
  }
  else if (currentPlayer == players[1].Player) {
    if ($("#button-player-2").hasClass("btn-warning")) {
      $("#button-player-2").removeClass("btn-warning");
      $("#button-player-2").addClass("btn-success");
      $("#button-player-2").closest(".card").addClass("current-player");
    }
    $("<span class='badge badge-light ml-2'>It's your turn!</span>").appendTo('#button-player-2');
    reloadPlayer2Required = true;
    reloadHandCards();
  }
  else if (currentPlayer == players[2].Player) {
    if ($("#button-player-3").hasClass("btn-warning")) {
      $("#button-player-3").removeClass("btn-warning");
      $("#button-player-3").addClass("btn-success");
      $("#button-player-3").closest(".card").addClass("current-player");
    }
    $("<span class='badge badge-light ml-2'>It's your turn!</span>").appendTo('#button-player-3');
    reloadPlayer3Required = true;
    reloadHandCards();
  }
  else if (currentPlayer == players[3].Player) {
    if ($("#button-player-4").hasClass("btn-warning")) {
      $("#button-player-4").removeClass("btn-warning");
      $("#button-player-4").addClass("btn-success");
      $("#button-player-4").closest(".card").addClass("current-player");
    }
    $("<span class='badge badge-light ml-2'>It's your turn!</span>").appendTo('#button-player-4');
    reloadPlayer4Required = true;
    reloadHandCards();
  }
  else{
      console.log("Error in: reloadPlayerBadge  occured");
  }
}


function reloadTopCard() {
    $('#topcard img').remove();
    showTopCard();
}

function reloadHandCards() {
    if (reloadPlayer1Required) {
        $("#handcards1 li").remove();
        showHandCards();
    }
    if (reloadPlayer2Required) {
        $("#handcards2 li").remove();
        showHandCards();
    }
    if (reloadPlayer3Required) {
        $("#handcards3 li").remove();
        showHandCards();
    }
    if (reloadPlayer4Required) {
        $("#handcards4 li").remove();
        showHandCards();
    }

    reloadPlayer1Required = false;
    reloadPlayer2Required = false;
    reloadPlayer3Required = false;
    reloadPlayer4Required = false;

}

function showTopCard() {
    let cardColor = null;
    if (13 == currentTopcard.Value || 14 == currentTopcard.Value) {
        cardColor = currentWildCardColor;
    } else {
        cardColor = currentTopcard.Color;
    }
    let showCard = convertCardObjectToImgPath(currentTopcard.Value, cardColor);
    $("<img class='mr-2' src='" + showCard + "'> </img>").appendTo('#topcard');
    // .animate( properties [, duration ] [, easing ] [, complete ] )

}

function showHandCards() {
    console.log(players);

    for (let playerIndex = 0; playerIndex < players.length; playerIndex++) {
        if (currentPlayer == players[playerIndex].Player) {

            if (0 == playerIndex && 1 > $('#handcards1').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {
                    let handCardToAppend = convertCardObjectToImgPath(players[playerIndex].Cards[handCardIndex].Value, players[playerIndex].Cards[handCardIndex].Color);

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards1');
                    $("<img class='mr-2' src='" + handCardToAppend + "'> </img>").data("value", players[playerIndex].Cards[handCardIndex].Value).data("color", players[playerIndex].Cards[handCardIndex].Color).appendTo(liItem);
                }
            } else if (1 == playerIndex && 1 > $('#handcards2').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {
                    let handCardToAppend = convertCardObjectToImgPath(players[playerIndex].Cards[handCardIndex].Value, players[playerIndex].Cards[handCardIndex].Color);

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards2');
                    $("<img class='mr-2' src='" + handCardToAppend + "'> </img>").data("value", players[playerIndex].Cards[handCardIndex].Value).data("color", players[playerIndex].Cards[handCardIndex].Color).appendTo(liItem);
                }
            } else if (2 == playerIndex && 1 > $('#handcards3').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {
                    let handCardToAppend = convertCardObjectToImgPath(players[playerIndex].Cards[handCardIndex].Value, players[playerIndex].Cards[handCardIndex].Color);

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards3');
                    $("<img class='mr-2' src='" + handCardToAppend + "'> </img>").data("value", players[playerIndex].Cards[handCardIndex].Value).data("color", players[playerIndex].Cards[handCardIndex].Color).appendTo(liItem);
                }
            } else if (3 == playerIndex && 1 > $('#handcards4').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {
                    let handCardToAppend = convertCardObjectToImgPath(players[playerIndex].Cards[handCardIndex].Value, players[playerIndex].Cards[handCardIndex].Color);

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards4');
                    $("<img class='mr-2' src='" + handCardToAppend + "'> </img>").data("value", players[playerIndex].Cards[handCardIndex].Value).data("color", players[playerIndex].Cards[handCardIndex].Color).appendTo(liItem);
                }
            }
        } else {
            if (0 == playerIndex && 1 > $('#handcards1').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards1');
                    $("<img class='mr-2' src='cards/back.svg'> </img>").appendTo(liItem);
                }
            } else if (1 == playerIndex && 1 > $('#handcards2').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards2');
                    $("<img class='mr-2' src='cards/back.svg'> </img>").appendTo(liItem);
                }
            } else if (2 == playerIndex && 1 > $('#handcards3').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards3');
                    $("<img class='mr-2' src='cards/back.svg'> </img>").appendTo(liItem);
                }
            } else if (3 == playerIndex && 1 > $('#handcards4').children().length) {
                for (let handCardIndex = 0; handCardIndex < players[playerIndex].Cards.length; handCardIndex++) {

                    let liItem = $("<li class='list-inline-item'> </li>").appendTo('#handcards4');
                    $("<img class='mr-2' src='cards/back.svg'> </img>").appendTo(liItem);
                }
            }
        }
    }
}


function gameEnded(winner, score) {
    $("#player-handcards li").remove();

    for (let index = 0; index < players.length; index++) {
        $("#player-handcards span").remove();
        if (winner == players[index].Player) { //<i class='fas fa-trophy'></i>
            if (0 == index) {
                $("#button-player-1").append("  <i class='fas fa-trophy'></i>");
                $('#handcards1').append("<li class='list-inline-item'> <i class='fas fa-trophy pr-2 winner-dance-left'></i> Winner !!! <i class='fas fa-trophy pl-2 winner-dance-right'></i></li>");
                $('#handcards1').addClass('end-of-game');
                $('#handcards1').closest('.card-header').addClass('winner');
            } else if (1 == index) {
                $("#button-player-2").append("  <i class='fas fa-trophy'></i>");
                $('#handcards2').append("<li class='list-inline-item'> <i class='fas fa-trophy pr-2 winner-dance-left'></i> Winner !!! <i class='fas fa-trophy pl-2 winner-dance-right'></i></li>");
                $('#handcards2').addClass('end-of-game');
                $('#handcards2').closest('.card-header').addClass('winner');
            } else if (2 == index) {
                $("#button-player-3").append("  <i class='fas fa-trophy'></i>");
                $('#handcards3').append("<li class='list-inline-item'> <i class='fas fa-trophy pr-2 winner-dance-left'></i> Winner !!! <i class='fas fa-trophy pl-2 winner-dance-right'></i></li>");
                $('#handcards3').addClass('end-of-game');
                $('#handcards3').closest('.card-header').addClass('winner');
            } else if (3 == index) {
                $("#button-player-4").append("  <i class='fas fa-trophy'></i>");
                $('#handcards4').append("<li class='list-inline-item'> <i class='fas fa-trophy pr-2 winner-dance-left'></i> Winner !!! <i class='fas fa-trophy pl-2 winner-dance-right'></i></li>");
                $('#handcards4').addClass('end-of-game');
                $('#handcards4').closest('.card-header').addClass('winner');
            }
        } else {
            if (0 == index) {
                $('#handcards1').append("<li class='list-inline-item'> Score: " + players[index].Score + "</li>");
                $('#handcards1').addClass('end-of-game');
            } else if (1 == index) {
                $('#handcards2').append("<li class='list-inline-item'> Score: " + players[index].Score + "</li>");
                $('#handcards2').addClass('end-of-game');
            } else if (2 == index) {
                $('#handcards3').append("<li class='list-inline-item'> Score: " + players[index].Score + "</li>");
                $('#handcards3').addClass('end-of-game');
            } else if (3 == index) {
                $('#handcards4').append("<li class='list-inline-item'> Score: " + players[index].Score + "</li>");
                $('#handcards4').addClass('end-of-game');
            }
        }
    }

}

function gameStart(aNameArray) {
    let baseURL = "https://nowaunoweb.azurewebsites.net";
    let request = $.ajax({
        url: baseURL + "/api/Game/Start", // WOHIN?da wollen wir posts hinschicken!! könnte CASE SENSITIVE sein!!!wird sich nicht ändern, kann man in variable packen --> baseURL
        method: 'POST', //WAS FÜR EINE METHODE? was wollen wir machen?? NICHT GET!!
        data: JSON.stringify(aNameArray),
        contentType: 'application/json', //da gibts einen defaultwert, der muss ünberschrieben werden! siehe https://api.jquery.com/jQuery.ajax/
        dataType: 'json' //was wir in der Arntwort zurückerwarten, da gibts einen defaultwert, der muss ünberschrieben werden!
    });
    request.done(function(data) { //wenn gültige antwort: ergebnis von request in object

        gameID = data.Id;
        currentPlayer = data.NextPlayer;
        players = data.Players;
        currentTopcard = data.TopCard;

        console.log("Game ID = ", gameID);
        console.log("Current Player = ", currentPlayer);
        console.log("Players = ", players);
        console.log("Response gameStart: ", data);

        reloadPlayerNames();
        reloadPlayerBadge();
        showTopCard();
        showHandCards();

        // drawCard();
        //  playCard();
        // getHandOfCurrentPlayer(player1)
    });
    request.fail(function(msg) { //wenn keine gültige antwort: callback falls was schiefgeht(keine gültoge karte, oder technischer fehler, o.ä.)
        gameID = 0;
        console.log("Error in request: ", msg, request);
    });

}

function getTopCard() {
    var baseURL = "https://nowaunoweb.azurewebsites.net";
    let request = $.ajax({
        url: baseURL + "/api/Game/TopCard/" + gameID,
        method: 'GET',
        dataType: 'json',
    });
    request.done(function(data) {
        console.log("Respone getTopCard: ", data);
        currentTopcard = data;
        reloadTopCard();
    });
    request.fail(function(msg) {
        console.log("Error in request: ", msg, request);
    });
}

function playCard(value, color, wildColor) {
    var baseURL = "https://nowaunoweb.azurewebsites.net";
    // /api/Game/PlayCard/{id}?value={value}&color={color}&wildColor={wildColor}
    //              {id}?value="3"&color="Blue"&wildColor=''
    //              {id}?value="Draw4"&color="Black"&wildColor="Blue"

    let request = $.ajax({
        url: baseURL + "/api/Game/PlayCard/" + gameID + "?value=" + value + "&color=" + color + "&wildColor=" + wildColor,
        method: 'PUT',
        contentType: 'application/json',
        dataType: 'json'
    });
    request.done(function(data) {
        console.log("Played matching card.");
        console.log("Respone playCard: ", data);
        if ("WrongColor" == data.error) {

            if (currentPlayer == players[0].Player) {
                if (!$("#handcards1").hasClass('wrong-card')) {
                    $("#handcards1").addClass("wrong-card");
                }
            } else if (currentPlayer == players[1].Player) {
                if (!$("#handcards2").hasClass('wrong-card')) {
                    $("#handcards2").addClass("wrong-card");
                }
            } else if (currentPlayer == players[2].Player) {
                if (!$("#handcards3").hasClass('wrong-card')) {
                    $("#handcards3").addClass("wrong-card");
                }
            } else if (currentPlayer == players[3].Player) {
                if (!$("#handcards4").hasClass('wrong-card')) {
                    $("#handcards4").addClass("wrong-card");
                }
            }


        } else {
            if (0 == data.Cards.length) {
                gameEnded(data.Player, data.Score);
            }
            else {
                getTopCard();


                currentPlayerReloadRequired();

                for (let index = 0; index < players.length; index++) {
                    getHandOfPlayer(players[index].Player);
                }
                currentPlayer = data.Player;

                reloadPlayerBadge();
            }
        }
    });
    request.fail(function(msg) {
        console.log("Error in request, card eventually not matched: ", msg, request);

        reloadPlayerBadge();
    });
}

function drawCard() {

    var baseURL = "https://nowaunoweb.azurewebsites.net";
    let request = $.ajax({
        url: baseURL + "/api/Game/DrawCard/" + gameID,
        method: 'PUT',
        contentType: 'application/json',
        dataType: 'json'
    });
    request.done(function(data) {
        console.log("Respone drawCard: ", data);

        currentPlayerReloadRequired();

        getHandOfPlayer(currentPlayer);
        currentPlayer = data.NextPlayer;
        reloadPlayerBadge();

        console.log("New current player: ", currentPlayer);

    });
    request.fail(function(msg) {
        console.log("Error in request: drawCard ", msg, request);
    });
}

// Game/DrawCard/{id}
function getHandOfPlayer(playerName) {
    var baseURL = "https://nowaunoweb.azurewebsites.net";
    let request = $.ajax({
        url: baseURL + "/api/Game/GetCards/" + gameID + "?playerName=" + playerName,
        method: 'GET',
        dataType: 'json',
    });
    request.done(function(data) {
        //console.log("Respone getHand: ", data);
        for (let index = 0; index < players.length; index++) {
            if (playerName == players[index].Player) {

                let checkLength = players[index].Cards.length;
                players[index].Cards = data.Cards;
                if (checkLength != players[index].Cards.length) {
                    if (0 == index) {
                        reloadPlayer1Required = true;
                    } else if (1 == index) {
                        reloadPlayer2Required = true;
                    } else if (2 == index) {
                        reloadPlayer3Required = true;
                    } else if (3 == index) {
                        reloadPlayer4Required = true;
                    }
                }
                reloadHandCards();
            }
        }

    });
    request.fail(function(msg) {
        console.log("Error in request getHandOfPlayer: ", msg, request);
    });
}

function convertCardObjectToImgPath(cardValue, cardColor) {

    let imgPath = "cards/";

    if ("Blue" == cardColor) {
        if (13 == cardValue) {
            imgPath = imgPath + "wild4_b.svg";
        } else if (14 == cardValue) {
            imgPath = imgPath + "wild_b.svg";
        } else {
            imgPath = imgPath + "b";
        }
    } else if ("Green" == cardColor) {
        if (13 == cardValue) {
            imgPath = imgPath + "wild4_v.svg";
        } else if (14 == cardValue) {
            imgPath = imgPath + "wild_v.svg";
        } else {
            imgPath = imgPath + "v";
        }
    } else if ("Yellow" == cardColor) {
        if (13 == cardValue) {
            imgPath = imgPath + "wild4_j.svg";
        } else if (14 == cardValue) {
            imgPath = imgPath + "wild_j.svg";
        } else {
            imgPath = imgPath + "j";
        }
    } else if ("Red" == cardColor) {
        if (13 == cardValue) {
            imgPath = imgPath + "wild4_r.svg";
        } else if (14 == cardValue) {
            imgPath = imgPath + "wild_r.svg";
        } else {
            imgPath = imgPath + "r";
        }
    } else if ("Black" == cardColor) {
        if (13 == cardValue) {
            imgPath = imgPath + "wild4.svg";
        } else if (14 == cardValue) {
            imgPath = imgPath + "wild.svg";
        }
    }

    if (0 == cardValue) {
        imgPath = imgPath + "0.svg";
    } else if (1 == cardValue) {
        imgPath = imgPath + "1.svg";
    } else if (2 == cardValue) {
        imgPath = imgPath + "2.svg";
    } else if (3 == cardValue) {
        imgPath = imgPath + "3.svg";
    } else if (4 == cardValue) {
        imgPath = imgPath + "4.svg";
    } else if (5 == cardValue) {
        imgPath = imgPath + "5.svg";
    } else if (6 == cardValue) {
        imgPath = imgPath + "6.svg";
    } else if (7 == cardValue) {
        imgPath = imgPath + "7.svg";
    } else if (8 == cardValue) {
        imgPath = imgPath + "8.svg";
    } else if (9 == cardValue) {
        imgPath = imgPath + "9.svg";
    } else if (10 == cardValue) {
        imgPath = imgPath + "draw2.svg";
    } else if (11 == cardValue) {
        imgPath = imgPath + "skip.svg";
    } else if (12 == cardValue) {
        imgPath = imgPath + "reverse.svg";
    }
    return imgPath;

}
